...
//Suppliers
  ...
  public RobotContainer() {
    ...
    if (DCTEnum == ControllerEnums.XboxController) {
      ...
      //Drive controls
      ControllerSidewaysAxisSupplier = () -> modifyAxis(-driverControllerXbox.getRawAxis(X_AXIS), 0);
      ControllerForwardAxisSupplier = () -> modifyAxis(-driverControllerXbox.getRawAxis(Y_AXIS), 0);
      ControllerZAxisSupplier = () -> modifyAxis(-driverControllerXbox.getRawAxis(XBOX_Z_AXIS), 0);
      ...
      AlgaeBargeSup = ()->driverControllerXbox.getPOV() == 90;
      SlowFrontSup = ()->false;
  ...
  SmartDashboard.putData(new LineUpBarge(driveTrain, ControllerSidewaysAxisSupplier));
    ...
    if(elevatorExists && coralEndeffectorExists && distanceSensorsExist && algaeEndeffectorExists){
      ...
      new Trigger(()->CoralPlaceTeleSupplier.getAsBoolean() && RobotState.getInstance().deliveringCoralHeight==ElevatorEnums.Reef1).whileTrue(
          new SequentialCommandGroup(
            ...
            new InstantCommand(()->coralEndDefector.stopCoralEndEffector()),
            new InstantCommand(()->driveTrain.drive(new ChassisSpeeds(-0.8, 0, 0)), driveTrain),
            new WaitCommand(()->0.5),
    ...
    if(elevatorExists){
      ...
      SmartDashboard.putData("ELEVATOR RESET", new ElevatorReset(elevator));
    }
  ...
  private void registerNamedCommands() {
    ...
    Command coralHandlingCommandL2;
    ...
    if(elevatorExists&&funnelIntakeExists&&coralEndeffectorExists&&algaeEndeffectorExists) {
      ...
      placeWithLineupLeftL2 = new PlaceCoralDuringLineupSequential(algaeEndDefector, driveTrain, elevator,
          coralEndDefector, () -> selectCommand(() -> true), () -> ElevatorEnums.Reef2);
      ...
      breakAlgaeTapeWhileDriving = new SequentialCommandGroup(
        new InstantCommand(()->driveTrain.drive(new ChassisSpeeds(2, 0, 0))),
        new InstantCommand(()->algaeEndDefector.runAlgaeEndDefector(ALGAE_SHOOT_SPEED), algaeEndDefector),
        new WaitCommand(()->0.15),
        new InstantCommand(()->algaeEndDefector.stopAlgaeEndDefectorHard(), algaeEndDefector),
        new InstantCommand(()->RobotState.getInstance().algaeTapeBroken = true));
      ...
      autoBargeShoot = new SequentialCommandGroup(
        new InstantCommand(()->System.out.println("barge shot group started")),
      ...
      placeWithLineupWithAlgaeL3 = new InstantCommand(()->System.out.println("attempted to create named command but subsytem did not exist"));
      placeWithLineupWithAlgaeL4 = new InstantCommand(()->System.out.println("attempted to create named command but subsytem did not exist"));
      autoBargeShoot = new InstantCommand(()->System.out.println("attempted to create named command but subsytem did not exist"));
      breakAlgaeTapeWhileDriving = new InstantCommand(()->System.out.println("attempted to create named command but subsytem did not exist"));
    }

    if(DrivetrainExists) {
      driveForwardForHalfSec = new SequentialCommandGroup(
        new InstantCommand(()->driveTrain.drive(new ChassisSpeeds(1, 0, 0))),
        new WaitUntilCommand(0.8)
      );
    } else {
      driveForwardForHalfSec = new InstantCommand(()->System.out.println("attempted to create named command but subsytem did not exist"));
    }
    ...
    if(coralEndeffectorExists && funnelIntakeExists) {
      ...
      turnOnIntake = new SequentialCommandGroup(
        new InstantCommand(()->funnelIntake.runFunnelSine()),
        new InstantCommand(()->coralEndDefector.runCoralEndEffector(CORAL_ENDEFFECTOR_SPEED))
      );
    } else {
    ...
    NamedCommands.registerCommand("IntakeAlignAndRaiseCoralL2", coralHandlingCommandL2);
    ...
    NamedCommands.registerCommand("BreakAlgaeTapeWhileDriving", breakAlgaeTapeWhileDriving);
  ...
  public void teleopInit() {
    new ParallelRaceGroup(
      new WaitUntil(()->RobotState.getInstance().algaeTapeBroken),
      new SequentialCommandGroup(
        new WaitCommand(()->0.1),
        new InstantCommand(()->algaeEndDefector.runAlgaeEndDefector(ALGAE_SHOOT_SPEED), algaeEndDefector),
        new WaitCommand(()->0.25),
  ...
  private void resetStatus() {
    ...
    RobotState.getInstance().coralAligned = false;
    if(coralEndeffectorExists) {
      coralEndDefector.stopCoralEndEffector();
    }
    if(funnelIntakeExists) {
      funnelIntake.stopFunnel();
    }
  ...
  public void teleopPeriodic() {
    ...
    logCurrentCommands();
    SmartDashboard.putNumber("limelight closest tag", limelight.getClosestTagDist(Vision.APRILTAG_LIMELIGHTA_NAME));
  }
