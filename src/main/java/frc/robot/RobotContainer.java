...
//Suppliers
  ...
  public RobotContainer() {
    ...
    if (DCTEnum == ControllerEnums.XboxController) {
      ...
      AlgaeBargeSup = driverControllerXbox::getXButton;
      SlowFrontSup = ()->false;
      ...
      AlgaeBargeSup = driverControllerPS4::getSquareButton;
  ...
  //if the elevator exists, return a zeroElevator command, then the auto. (if the elevator is set up right, the zeroElevator command will be instant)
    if(elevatorExists) {
      return new SequentialCommandGroup(
        new SequentialCommandGroup(
          new InstantCommand(()->elevator.setElevatorPositionDynamicConfigs(HEIGHT_AT_LIMIT_SWITCH-1, MOTION_MAGIC_ELEVATOR_ACCLERATION/5, MOTION_MAGIC_ELEVATOR_VELOCITY/5, MOTION_MAGIC_ELEVATOR_JERK/5)),
